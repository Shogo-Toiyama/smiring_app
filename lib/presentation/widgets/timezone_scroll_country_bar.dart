...
import 'package:smiring_app/application/state/timedifference_provider/timedifference_scroll_providers.dart';
...
class TimezoneScrollCountryBar extends HookConsumerWidget {
      ...
      child: Row(
        ...
        children: [
          ...
          Row(
            ...
            children: [
              buildDropdown("year", 2020, 2030, selectedValues.value["year"]!, updateTime),
              const Text(
                ' / ',
                style: TextStyle(fontSize: 20),
              ),
              buildDropdown("month", 1, 12, selectedValues.value["month"]!, updateTime),
              const Text(
                ' / ',
                style: TextStyle(fontSize: 20),
              ),
              buildDropdown("day", 1, 31, selectedValues.value["day"]!, updateTime),
              Text(
                ' ${weekdaysList[now.weekday]}   ',
                style: TextStyle(fontSize: 15, color: now.weekday == 6 ? Colors.blue : now.weekday == 7 ? Colors.red : Colors.grey),
              ),
              buildDropdown("hour", 0, 23, selectedValues.value["hour"]!, updateTime),
              const Text(
                ' : ',
                style: TextStyle(fontSize: 20),
              ),
              buildDropdown("minute", 0, 59, selectedValues.value["minute"]!, updateTime),
            ],
          ),
        ],
      ),
...
Widget buildDropdown(String key, int min, int max, String currentValue, Function(String, String) onChanged) {
